...
var getCategories = handler.getAll(Category);
...
function* createCategory() {
  ...
  body = _.pick(body, ['title', 'slug', 'sort', 'meta', 'media']);
...
function* updateCategory() {
  ...
  if (body.slug !== category.get('slug')) {
...
function* deleteCategory() {
  /* jshint validthis: true */
  var category = yield getCategory.call(this);

  if (!category) return;

  this.throw(403, 'Cannot delete category!');
}
